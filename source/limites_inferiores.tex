...
onde $n^{\prime}$ é o número de variáveis projetadas após a última projeção $\rho_{d - 1}$ e ambas igualdades acontecem com alta probabilidade sobre as possíveis projeções $\rho_{1}, \rho_{2}, \dots, \rho_{d - 1}$. Então nós mostramos que $\Psi(f^{m, d})$ e $\Psi(C)$ em si não são correlacionadas para mostrar que $f^{m, d}$ e $C$ também não são correlacionadas.
...
Seja $\rho_{1} \leftarrow P_{1}$ e para todo $a \in A_{1}$ seja $S_{a} = \{a^{\prime} \in \inputt(a) \lvert \rho_{1}(a^{\prime}) = x_{a} \}$. Nós dizemos que $\rho_{1}$ é típica se:

\begin{enumerate}

	\item Para todo $a \in A_{1}$, $\big\lvert \lvert S_{a} \rvert - qw \big\rvert \geq w^{g(1, d)}$, onde definimos $g(i, d) = 1/4 + (i - 1)/40d$ para todo $i \in [d - 1]$.
	
	\item Para todo $a \in A_{1}$, $\rho_{1}(x_{a}) = x_{a}$.
...
Para mostrar que $f^{m, d}$ mantém estrutura nós primeiro argumentamos que $\rho_{1} \leftarrow P_{1}$ é típica com alta probabilidade.

\begin{prop}

Seja $\rho_{1} \leftarrow P_{1}$ e $a \in A_{2}$, então:

\begin{enumerate}

	\item $\Pr \Big[\big\lvert \lvert S_{a} \rvert - qw \big\rvert \leq w^{g(1, d)} \Big] = 1 - e^{-\widetilde{\Omega}(w^{1/4})}$;
	
	\item $\Pr[\rho_{1}(x_{a^{\prime}}) = x_{a^{\prime}} \text{ para todo } x_{a^{\prime}} \in \inputt(a) ] = 1 - \widetilde{\mathcal{O}}(w^{-1/4})$.
...
\begin{enumerate}
	...
	\begin{IEEEeqnarray*} {rCl}
		\sum_{\rho_{1} \in \mathcal{B}} \kappa^{s} \Pr_{\boldsymbol{\rho} \leftarrow P_{1}}[\boldsymbol{\rho} = \rho_{1}^{\prime}] & \leq & (16\kappa 2^{r}r)^{s}\sum_{\rho_{1}^{\prime} \in \mathcal{C}} \Pr_{\boldsymbol{\rho} \leftarrow P_{1}}[\boldsymbol{\rho} = \rho_{1}^{\prime}] \\
																					    & \leq & (16\kappa 2^{r}r)^{s}.  
	\end{IEEEeqnarray*}
...
\begin{proof} (Prova do Lema \ref{projection_switching_lemma_Pi}.)
	...
	\begin{IEEEeqnarray*} {rCl}
		Pr_{\boldsymbol{\rho} \leftarrow P_{i}}[\boldsymbol{\rho} \in \mathcal{B}_{i}] & \leq & (16\kappa 2^{r}r)^{s} \sum_{\rho_{1}^{\prime} \in \mathcal{C}} \Pr_{\boldsymbol{\rho} \leftarrow P_{i}}[\boldsymbol{\rho} = \rho_{i}^{\prime}] \\
														& \leq & (16\kappa 2^{r}r)^{s}
	\end{IEEEeqnarray*}
...
Assim como fizemos na prova do Teorema \ref{parity_lb_2}, nós iremos provar o Teorema \ref{Sipser_f_lb_2} em duas partes em que mostramos o seguinte em cada uma delas. Seja $C$ um circuito de profundidade $d$, tamanho $S = 2^{n^{\Theta(\frac{1}{d - 1})}}$ e com fan-in polilogaritmico no teu nível mais baixo:
